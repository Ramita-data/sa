...
function preload() {
  // Heroes
  for (let i = 1; i <= 4; i++) {
    this.load.spritesheet(`hero${i}`, `hero${i}_spritesheet.png`, { frameWidth: 1080, frameHeight: 1080 });
  }

  // Monsters
  for (let i = 1; i <= 4; i++) {
    this.load.spritesheet(`monster${i}`, `monster${i}_spritesheet.png`, { frameWidth: 1080, frameHeight: 1080 });
  }
...
function create() {
  ...
  let worldKey = localStorage.getItem('castle') === 'unlocked' ? 'castle' :
                 localStorage.getItem('mystic') === 'unlocked' ? 'mystic' :
                 'village';
  this.add.image(960, 540, worldKey);
  ...
  let heroKey = localStorage.getItem('hero3') === 'unlocked' ? 'hero3' :
                localStorage.getItem('hero4') === 'unlocked' ? 'hero4' :
                'hero1';
  player = this.physics.add.sprite(400, 300, heroKey);
  ...
  const createMonster = (key, x, y, flying=false) => {
    let m = this.physics.add.sprite(x, y, key);
...
function update() {
  ...
  if (cursors.left.isDown) {
    ...
    player.setVelocityY(cursors.up.isDown ? -300 : 300);
    player.anims.play(`${heroKey}_walk`, true);
